export class CheckboxGridField {
  static create(question, value = {}) {
    ...
    rows.forEach((r, ri) => {
      ...
      cols.forEach((c) => {
        ...
        input.type = 'checkbox';
        input.className = 'gf-grid-checkbox-input';
        input.name = `q-${question.questionId}_r${ri}[]`;
        input.value = c;
        const existing = value[String(ri)] || [];
        if (Array.isArray(existing) && existing.includes(c)) input.checked = true;

        const visual = document.createElement('div');
        visual.className = 'gf-grid-checkbox-visual';
